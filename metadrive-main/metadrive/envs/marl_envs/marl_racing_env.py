...
RACING_CONFIG = dict(
    ...
    num_agents=12,
    # exit_length controls spawn slots: max agents ~= lane_num * floor((exit_length-10)/8).
    # With lane_num=2, exit_length=60 supports up to 12 agents (see test_ma_racing.py).
...
def export_map_png(output_path="marl_racing_map.png", resolution=(2048, 2048), seed=0):
    """Export the racing map top-down view to a PNG file."""
    import cv2
    from metadrive.utils.draw_top_down_map import draw_top_down_map

    env = MultiAgentRacingEnv(dict(use_render=False, num_agents=1))
    try:
        env.reset(seed=seed)
        img = draw_top_down_map(env.current_map, resolution=resolution, semantic_map=True)
        cv2.imwrite(output_path, img)
        print(f"Map PNG saved: {output_path} ({resolution[0]}x{resolution[1]})")
        return output_path
    finally:
        env.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Multi-agent racing env demo")
    parser.add_argument("--map", action="store_true", help="Export map top-down PNG and exit")
    parser.add_argument("--output", default="marl_racing_map.png", help="PNG output path for --map")
    parser.add_argument("--seed", type=int, default=0, help="Map seed for --map")
    parser.add_argument("--video", action="store_true", help="Record BEV + interface videos in _vis()")
    args = parser.parse_args()

    if args.map:
        export_map_png(output_path=args.output, seed=args.seed)
    else:
        _vis(generate_video=args.video)
